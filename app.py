...
class MainApp:
    def __init__(self, page: ft.Page):
        ...
        self.logout_button = ft.IconButton(ft.Icons.EXIT_TO_APP, on_click=self.logout) # Usamos un icono de salir
        self.update_appbar()
        self.update_view()
        self.page.add(
            ft.Column(
    ...
    def login(self, e):
        ...
        if user_data and verify_password(user_data[2], password):
            ...
            self.navigation_bar.destinations[3].disabled = False
            self.update_appbar()
            self.page.go("/home")
            self.page.update()
        else:
    ...
    def update_appbar(self):
        if self.logged_in:
            self.page.appbar = ft.AppBar(
                ...
                actions=[self.logout_button], # Mostrar el botón de salir si está logueado
            )
        else:
            self.page.appbar = ft.AppBar(
                title=ft.Text(self.page.title),
                actions=[self.user_icon_button], # Mostrar el icono de usuario si no está logueado
            )
        self.page.update()
